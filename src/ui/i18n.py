...
EN = {
    ...
    # --- faq + persistencia ---
    "Preguntas metodológicas frecuentes": "Frequently asked methodology questions",
    "**¿Cómo se simulan las colas? ¿Usan el coeficiente de variación?** No. Un CV resume toda la variabilidad en un número y presupone una forma (aprox. simétrica) — justo lo que falla en la cola. Acá la cola sale de tres piezas: (1) los años malos reales del departamento, normalizados a tecnología actual, que quedan como anclas; (2) la forma completa de la distribución ajustada a esos shocks (weibull, kernel, etc.), seleccionada por backtest con gates que rechazan a quien comprima la cola; (3) la masa en cero. El CV del departamento queda implícito en la distribución; nunca se usa el de la provincia.":
        "**How are tails simulated? Do you use the coefficient of variation?** No. A CV compresses all variability into one number and presumes a roughly symmetric shape — exactly what fails in the tail. Here the tail comes from three pieces: (1) the department's real bad years, normalized to current technology, kept as anchors; (2) the full shape of the distribution fitted to those shocks (weibull, kernel, etc.), selected by backtest with gates that reject tail compression; (3) the zero mass. The department's CV is implicit in the distribution; the province's CV is never used.",
    "**¿Los datos son del departamento o de la provincia?** La distribución se ajusta SIEMPRE a la serie del departamento seleccionado. La provincia no entra en la forma de la cola; solo aparece en dos lugares acotados: el prior de P(Y=0) por nivel de agregación, y (en Resultados) la metodología modal si activás el modo uniforme. Si simulás a nivel FIELD/FARM se aplican multiplicadores de volatilidad prior — un supuesto documentado en config, no una estimación.":
        "**Is the data from the department or the province?** The distribution is ALWAYS fitted to the selected department's series. The province never shapes the tail; it appears in only two bounded places: the P(Y=0) prior by aggregation level, and (in Results) the modal methodology when uniform mode is on. Simulating at FIELD/FARM level applies prior volatility multipliers — a documented assumption in config, not an estimate.",
    "**¿Cómo se generan los rindes cero?** Con un modelo hurdle en dos partes: P(Y=0) se estima clasificando los ceros históricos por superficie (sembrada sin cosechar ≠ error de encuesta) y aplicando shrinkage beta-binomial hacia un prior por nivel de agregación — por eso P(Y=0)>0 aunque el departamento nunca haya registrado un cero. En la simulación: si u<P(Y=0) el rinde es exactamente 0; si no, se muestrea de la distribución positiva. Una densidad continua pura no puede producir un cero verdadero; la mezcla sí.":
